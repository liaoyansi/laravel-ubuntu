    ...
    <div class="column">
        ...
        <form class="ui large form"  method="POST" action="{{ url('admin/login') }}">
            ...
            <div class="ui stacked segment ">
                ...
                <div class="ui error message {{ count($errors->login)>0 ? 'visible' : 'hidden' }}">
                    <p><strong>{{$errors->login->first()}}</strong></p>
                </div>
                <div class="field {{ $errors->login->has('username') ? 'error' : '' }}">
                    <div class="ui left icon input">
                        <i class="user icon"></i>
                        <input type="text" name="username" placeholder="用户名" value="{{ old('username') }}">
                    </div>
                </div>
                <div class="field  {{ $errors->login->has('password') ? 'error' : '' }}">
                    <div class="ui left icon input">
                        <i class="lock icon"></i>
